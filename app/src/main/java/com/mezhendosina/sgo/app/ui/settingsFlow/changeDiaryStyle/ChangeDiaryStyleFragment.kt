...
package com.mezhendosina.sgo.app.ui.settingsFlow.changeDiaryStyle
...
import com.mezhendosina.sgo.app.ui.journalFlow.journalItem.adapters.HomeworkAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            SettingsDataStore.DIARY_STYLE.getValue(requireContext(), DiaryStyle.AS_CARD).collect {
                if (binding != null) withContext(Dispatchers.Main) {
                    when (it) {
                        DiaryStyle.AS_CARD -> {
        ...
        binding!!.diaryDesignRadioGroup.setOnCheckedChangeListener { _, i ->
            CoroutineScope(Dispatchers.Main).launch {
                SettingsDataStore.DIARY_STYLE.editPreference(
                    requireContext(),
                    when (i) {
                        binding!!.asList.id -> DiaryStyle.AS_LIST
                        binding!!.withDrawable.id -> DiaryStyle.AS_CARD
                        else -> DiaryStyle.AS_CARD
                    }
                )
                Singleton.diaryStyle.value =
                    SettingsDataStore.DIARY_STYLE.getValue(requireContext(), DiaryStyle.AS_CARD)
                        .first()
            }
        }
